...
class AMPDiscriminator(nn.Module):
    def __init__(
            self, input_dim, amp_reward_coef, hidden_layer_sizes, device, task_reward_lerp=0.0, dropout_prob=0.0):
        ...
        for hidden_dim in hidden_layer_sizes:
            ...
            if dropout_prob > 0.0:
                amp_layers.append(nn.Dropout(p=dropout_prob))
    ...
    def compute_grad_pen(self,
        ...
        # grad_pen = lambda_ * torch.clip((grad.norm(2, dim=1) - 1).pow(2), min=0.).mean()
        grad_pen = lambda_ * torch.sum(torch.square(grad), dim=-1).mean()
        return grad_pen

    def predict_amp_reward(
            self, state, next_state, task_reward):
        with torch.no_grad():
            self.eval()
