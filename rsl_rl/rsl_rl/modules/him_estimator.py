...
class PIMEstimator(nn.Module):
    def __init__(self,
        ...
        self.num_one_step_obs = num_one_step_obs
        self.num_latent = enc_hidden_dims[-1]
        self.num_perception = num_perception
        self.max_grad_norm = max_grad_norm
        self.temperature = temperature

        # Encoder
        enc_input_dim = self.temporal_steps * self.num_one_step_obs + self.num_perception
        enc_layers = []
        for l in range(len(enc_hidden_dims) - 1):
            enc_layers += [nn.Linear(enc_input_dim, enc_hidden_dims[l]), activation]
            enc_input_dim = enc_hidden_dims[l]
        ...
        # Target
        tar_input_dim = self.num_one_step_obs + self.num_perception
        tar_layers = []
        for l in range(len(tar_hidden_dims)):
            tar_layers += [nn.Linear(tar_input_dim, tar_hidden_dims[l]), activation]
            tar_input_dim = tar_hidden_dims[l]
    ...
    def get_latent(self, obs_history, perception):
        vel, z = self.encode(torch.cat((obs_history, perception), dim=-1))
        return vel.detach(), z.detach()

    def forward(self, obs_history, perception):
        parts = self.encoder(torch.cat((obs_history, perception), dim=-1).detach())
        vel, z = parts[..., :3], parts[..., 3:]
        z = F.normalize(z, dim=-1, p=2)
        return vel.detach(), z.detach()

    def encode(self, obs_history, perception):
        parts = self.encoder(torch.cat((obs_history, perception), dim=-1).detach())
        vel, z = parts[..., :3], parts[..., 3:]
        z = F.normalize(z, dim=-1, p=2)
        return vel, z

    def update(self, obs_history, next_critic_obs, perception, lr=None):
        if lr is not None:
            self.learning_rate = lr
            for param_group in self.optimizer.param_groups:
                param_group['lr'] = self.learning_rate
        ...
        z_s = self.encoder(torch.cat((obs_history, perception), dim=-1))
        z_t = self.target(torch.cat((next_obs, perception), dim=-1))
        pred_vel, z_s = z_s[..., :3], z_s[..., 3:]
